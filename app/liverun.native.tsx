...
import LiveRunMap from '../components/LiveRunMap.native';
...
export default function LiveRunScreen() {
    ...
    const [runSessionRating, setRunSessionRating] = useState<number | null>(null);
    const [runSessionRatingLoading, setRunSessionRatingLoading] = useState(false);
    const [runSessionRatingSaved, setRunSessionRatingSaved] = useState(false);
    const runSessionRatingSavedRef = useRef(false);
    ...
    const handleRateLastRunSession = async (rating: number) => {
        if (runSessionRatingLoading || runSessionRatingSaved) return;
        if (!lastSessionSummary?.sessionId) {
            Alert.alert('Error', 'No se encontró la sesión para valorar.');
            return;
        }
        ...
        try {
            ...
            await rateRunSession(lastSessionSummary.sessionId, rating);

            runSessionRatingSavedRef.current = true;
            setRunSessionRatingSaved(true);
        } catch (error) {
            console.error('Error guardando valoración de running:', error);
    ...
    const ensureRunSessionRatingBeforeClose = async () => {
        if (runSessionRatingSavedRef.current) return;
    ...
    const finish = async (options?: { showSummary?: boolean; goHome?: boolean }) => {
        ...
        try {
            if (startedAtMs) {
                const createdSession = await createRunSession({
                ...
                createdSessionId = createdSession?.item?.id ?? null;
            }
        } catch (error) {
        ...
        runSessionRatingSavedRef.current = false;
                    ...
                    >
                        ...
                        >
                            {!runSessionRatingSaved && (
                                ...
                                >
                                    ¿Cómo valorarías el esfuerzo de esta sesión?
                                </Text>
                            )}
                            ...
                            {runSessionRatingLoading ? (
                                ...
                                <View style={{ alignItems: 'center', paddingVertical: 8 }}>
                                    ...
                                    >
                                        Aportará datos a tus estadísticas.
                                    </Text>
                                </View>
                            ) : (
                                <>
                                    ...
                                    >
                                        {Array.from({ length: 10 }, (_, index) => {
                                            const value = index + 1;
                                            const selected = runSessionRating === value;

                                            return (
                                                <Pressable
                                                    key={value}
                                                    onPress={() => handleRateLastRunSession(value)}
                                                    style={{
                                                        width: 20,
                                                        height: 20,
                                                        borderRadius: 5,
                                    ...
                                    >
                                        Si no calificas, se guardará con 6.0 automáticamente.
                                    </Text>
                                </>
                            )}
                        </View>
